...
def plot_observed_vs_predicted(models, X_train, y_train, X_validate, y_validate,
                               numeric_strategy="median", categorical_strategy="most_frequent",
                               ncols=2, title="Observed vs Predicted — Validation Set"):
    ...
    for ax, (name, model) in zip(axes, models.items()):
        pipeline, _, _, _, _ = fit_and_evaluate_model(
            model=model,
            X_train=X_train,
            y_train=y_train,
            X_validate=X_validate,
        ...
        ax.scatter(y_validate, y_pred, alpha=0.7)

        min_val = min(np.min(y_validate), np.min(y_pred))
        max_val = max(np.max(y_validate), np.max(y_pred))
...
def collect_bootstrap_distributions(models, X_train, y_train, X_validate, y_validate):
    dist_dict = {}

    for name, model in models.items():
        _, _, _, _, boot_val_dist = fit_and_evaluate_model(
            model=model,
            X_train=X_train,
            y_train=y_train,
            X_validate=X_validate,
